	...
	<script type="text/javascript">
		$(document).ready(function(e) {
	        $('.view_video').click(function(){
				var src = 'http://www.youtube.com/embed/'+$(this).data('link');
				$('.left_video iframe').attr('src',src);
			});
	    });
	</script>
	<div class="video_popup">
		<iframe title="<?=$videonb[0]['ten'.$lang]?>" width="100%" src="http://www.youtube.com/embed/<?=$func->getYoutube($videonb[0]['link_video'])?>" frameborder="0" allowfullscreen></iframe></div>
	<div id="clickvideo">
	    <?php for($i=0,$count_video=count($videonb);$i<$count_video;$i++){?>
	       <div>
	        	<div style="width: 100%;margin-bottom: 10px;">
		        	<a class="view_video" data-link="<?=$func->getYoutube($videonb[$i]['link_video'])?>" href="javascrit:void(0)" title="<?=$videonb[$i]['ten'.$lang]?>">
			            <img src="http://img.youtube.com/vi/<?=$func->getYoutube($videonb[$i]['link_video'])?>/0.jpg" alt="<?=$videonb[$i]['ten'.$lang]?>" />
			        </a>
		        </div>
	        </div>
	    <?php } ?>
