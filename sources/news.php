<?php  
	...
	if($id!='')
	{
		...
		$filehosonangluc = $d->rawQuery("select ten$lang as ten,taptin from #_gallery where id_photo = ? and com='news' and type = ? and kind='man' and val = ? and hienthi > 0 order by stt,id desc",array($row_detail['id'],$type,'taptin'));
	...
	{
		/* Lấy cấp 1 detail */
		$news_list = $d->rawQueryOne("select id, ten$lang as ten, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_list where id = ? and type = ? limit 0,1",array($idl,$type));
	...
	{
		/* Lấy cấp 2 detail */
		$news_cat = $d->rawQueryOne("select id, id_list, ten$lang as ten, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_cat where id = ? and type = ? limit 0,1",array($idc,$type));
	...
	{
		/* Lấy cấp 3 detail */
		$news_item = $d->rawQueryOne("select id, id_list, id_cat, ten$lang as ten, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_item where id = ? and type = ? limit 0,1",array($idi,$type));
	...
	{
		/* Lấy cấp 4 */
		$news_sub = $d->rawQueryOne("select id, id_list, id_cat, id_item, ten$lang as ten, tenkhongdauvi, tenkhongdauen, type, photo, options from #_news_sub where id = ? and type = ? limit 0,1",array($ids,$type));
