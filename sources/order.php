<?php
  ...
  // $breadcrumbs = $breadcr->getBreadCrumbs();
	...
	if(isset($_POST['thanhtoan']))
	{
		...
		$ghichu = (isset($_POST['ghichu']) && $_POST['ghichu'] != '') ? htmlspecialchars($_POST['ghichu']) : '';

		$tamtinh = (isset($_POST['price-temp']) && $_POST['price-temp'] > 0) ? htmlspecialchars($_POST['price-temp']) : 0;
		$ship = (!empty($_POST['price-ship']) && $_POST['price-ship'] > 0) ? htmlspecialchars($_POST['price-ship']) : 0;
		$discount = (!empty($_POST['price-discount']) && $_POST['price-discount'] > 0) ? htmlspecialchars($_POST['price-discount']) : 0;
		$total = (!empty($_POST['price-total']) && $_POST['price-total'] > 0) ? htmlspecialchars($_POST['price-total']) : 0;
													...
													<table border="0" cellpadding="0" cellspacing="0" width="100%">
														...
														<tbody>
															...
															<tr>
																...
																if($discount)
																{
																	$contentAdmin.='<br><strong>Phiếu giảm giá: </strong> '.$func->format_money($discount);
																}
											...
											<table border="0" cellpadding="0" cellspacing="0" width="100%">
												...
												<tbody>
													...
													<tr>
														...
														if($discount)
														{
															$contentCustomer.='<br><strong>Phiếu giảm giá: </strong> '.$func->format_money($discount);
														}
		...
		//$data_donhang['magiamgia'] = $discount;
		...
		if($id_insert)
		{
			for($i=0;$i<$max;$i++)
			{
				...
				if($q>0){
					/* Cập nhật số lượng */
					$row_data_get = $d->rawQueryOne("select type,soluong from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($pid,'san-pham'));
					$data_update_quantity['soluong'] = $row_data_get['soluong'] - $q;
					$d->where('id',$pid);
					$d->update('product',$data_update_quantity);
				}
