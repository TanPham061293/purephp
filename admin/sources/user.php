		...
		if(!empty($_POST))
		{
			...
			if(isset($changepass) && $changepass == 1)
			{
				...
				if($old_pass!='' || $new_pass!='' ||  $renew_pass!='')
				{
					...
					if($new_pass!="")
					{
						if($new_pass=='123qwe' || $new_pass=='123456' || $new_pass=='bmwebco') $func->transfer("Mật khẩu bạn đặt quá đơn giãn, xin vui lòng chọn mật khẩu khác","index.php?com=user&act=admin_edit&changepass=1", false);			
