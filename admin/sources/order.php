<?php
	...
	function save_item()
	{
		...
		if($_POST['tinhtrang']==5 && $_POST['dahuy'] < 1){
			$sql_dh = "select id_product,soluong from #_order_detail where id_order = ".$id." ";
			$items_donhang = $d->rawQuery($sql_dh);
			foreach ($items_donhang as $key => $value) {
				$row_data_get = $d->rawQueryOne("select type,soluong from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($value['id_product'],'san-pham'));
				$data_update_quantity['soluong'] = $row_data_get['soluong'] + $value['soluong'];
				$d->where('id',$value['id_product']);
