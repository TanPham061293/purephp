<?php
	...
	switch($act)
	{
		...
		case "man_coupon":
			get_coupons();
			$template = "product/coupon/coupon";
			break;
		case "add_coupon":
			$template = "product/coupon/coupon_add";
			break;
		case "edit_coupon":
			get_coupon();
			$template = "product/coupon/coupon_add";
	...
	function save_item()
	{
		...
		if($data)
		{
			...
			$checkQuantity = $d->rawQueryOne("select soluong,lichsunhap from #_product where id = ? and type = ?  order by stt,id desc limit 0,1",array($id,$type));
			...
			//print_r($checkQuantity);
			if($data['soluongnhap'] > 0){

				$data['soluong'] = ($data['soluongnhap'] + $checkQuantity['soluong']);
				$data['soluongnhap'] = $data['soluongnhap'];
				if($checkQuantity['lichsunhap']!=''){
					$data['lichsunhap'] = $checkQuantity['lichsunhap'].','.time().'-'.$data['soluongnhap'];
				}else{
					$data['lichsunhap'] = time().'-'.$data['soluongnhap'];
				}
			}
		...
		if($id && $act!='save_copy')
		{
			...
			if($data['masp'] !=''){
				$data['linkqr'] = 'check?qrcode='.$data['masp'];
			}
			$data['ngaysua'] = time();
			$d->where('id', $id);
			$d->where('type', $type);
	...
	function get_coupons()
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product_coupon where type = ? $where order by stt,id desc $limit";
		$items = $d->rawQuery($sql,array($type));
		$sqlNum = "select count(*) as 'num' from #_product_coupon where type = ? $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_coupon&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}
	/* Edit coupon */
	function get_coupon()
	{
		global $d, $func, $strUrl, $curPage, $item, $gallery, $type, $com;
		$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage.$strUrl, false);
		$item = $d->rawQueryOne("select * from #_product_coupon where id = ? and type = ? limit 0,1",array($id,$type));
		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage.$strUrl, false);
	}
	/* Save coupon */
	function save_coupon()
	{
		global $d, $curPage, $func, $config, $com, $type;
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage, false);
		/* Post dữ liệu */
		$data = (isset($_POST['data'])) ? $_POST['data'] : null;
		if($data)
		{
			...
			$data['hienthi'] = (isset($data['hienthi'])) ? 1 : 0;
			$data['giatri'] = (isset($data['giatri']) && $data['giatri'] != '') ? str_replace(",","",$data['giatri']) : 0;
			$data['type'] = $type;
		}
		/* Post seo */
		if(isset($config['product'][$type]['seo_coupon']) && $config['product'][$type]['seo_coupon'] == true)
		{
			$dataSeo = (isset($_POST['dataSeo'])) ? $_POST['dataSeo'] : null;
			if($dataSeo)
			{
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES['file']["name"]);
				if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_coupon'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_product_coupon where id = ? and type = ? limit 0,1",array($id,$type));
					if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				}
			}
			$data['ngaysua'] = time();
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('product_coupon',$data))
			{
				/* SEO */
				if(isset($config['product'][$type]['seo_coupon']) && $config['product'][$type]['seo_coupon'] == true)
				{
					$d->rawQuery("delete from #_seo where idmuc = ? and com = ? and act = ? and type = ?",array($id,$com,'man_coupon',$type));
					$dataSeo['idmuc'] = $id;
					$dataSeo['com'] = $com;
					$dataSeo['act'] = 'man_coupon';
					$dataSeo['type'] = $type;
					$d->insert('seo',$dataSeo);
				}
				$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			$data['ngaytao'] = time();
			if($d->insert('product_coupon',$data))
			{
				$id_insert = $d->getLastInsertId();
				/* SEO */
				if(isset($config['product'][$type]['seo_coupon']) && $config['product'][$type]['seo_coupon'] == true)
				...
				$func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage);
			}
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage, false);
		}
	}
	/* Delete coupon */
	function delete_coupon()
	{
		...
		if($id)
		{
			...
			{
				$func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				$d->rawQuery("delete from #_product_coupon where id = ?",array($id));
				$func->transfer("Xóa dữ liệu thành công", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage.$strUrl);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage.$strUrl, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_coupon&type=".$type."&p=".$curPage.$strUrl, false);
	}
